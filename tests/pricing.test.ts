...
beforeAll(async () => {
  ...
  // vatRateBps: 0 throughout this block — these fixtures are about item/
...
describe('priceCart VAT extraction', () => {
  ...
  it('two lines at different rates produce two breakdown entries', async () => {
    const result = await priceCart(restaurantId, [
      { menuItemId: foodItemId, quantity: 1, selectedOptionIds: [] }, // 10%
      { menuItemId: plainItemId, quantity: 1, selectedOptionIds: [] }, // 0% — from the outer beforeAll
    ]);
  ...
  it('a menu item created without specifying vatRateBps defaults to 1000 (10%)', async () => {
    const item = await db.menuItem.create({
      data: {
        restaurantId,
        categoryId: (await db.menuCategory.findFirstOrThrow({ where: { restaurantId } })).id,
        name: 'Default Rate Item',
        priceCents: 1100,
      },
    });

    expect(item.vatRateBps).toBe(1000);
  });
});
